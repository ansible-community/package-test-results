...
As per the [Ansible community package inclusion requirements][ci-testing], collections must pass `ansible-test sanity` tests. Version `5.11.0` of `theforeman.foreman`, corresponding to the `v5.11.0` tag in this repo, fails one or more of the required sanity tests.
...
The test `ansible-test sanity --test yamllint` [[explain](https://docs.ansible.com/ansible-core/2.21/dev_guide/testing/sanity/yamllint.html)] failed with 107 errors:
