...
The contents in the `v6.9.0` git tag do not match `check_point-mgmt-6.9.0.tar.gz` as uploaded to Ansible Galaxy. For future releases, please make sure that the contents uploaded to Galaxy match the sources that were tagged as that release. See the [Repository management requirements][repo-mgmt] for more information.
...
The following files differ between the `v6.9.0` git tag and `check_point-mgmt-6.9.0.tar.gz` on Ansible Galaxy:
