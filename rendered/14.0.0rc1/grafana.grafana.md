...
As per the [Ansible community package inclusion requirements][ci-testing], collections must pass `ansible-test sanity` tests. Version `6.1.0` of `grafana.grafana`, corresponding to the `6.1.0` tag in this repo, fails one or more of the required sanity tests.
...
The test `ansible-test sanity --test yamllint` [[explain](https://docs.ansible.com/ansible-core/2.21/dev_guide/testing/sanity/yamllint.html)] failed with 4 errors:
...
roles/grafana/tasks/install.yml:33:26: colons: too many spaces before colon
roles/mimir/defaults/main.yml:31:1: empty-lines: too many blank lines (1 > 0)
```
