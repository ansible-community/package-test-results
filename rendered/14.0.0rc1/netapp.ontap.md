...
As per the [Ansible community package inclusion requirements][ci-testing], collections must pass `ansible-test sanity` tests. Version `23.5.0` of `netapp.ontap`, corresponding to the `23.5.0` tag in this repo, fails one or more of the required sanity tests.

The contents in the `23.5.0` git tag do not match `netapp-ontap-23.5.0.tar.gz` as uploaded to Ansible Galaxy. For future releases, please make sure that the contents uploaded to Galaxy match the sources that were tagged as that release. See the [Repository management requirements][repo-mgmt] for more information.
...
The test `ansible-test sanity --test validate-modules` [[explain](https://docs.ansible.com/ansible-core/2.21/dev_guide/testing/sanity/validate-modules.html)] failed with 4 errors:
...
The following files differ between the `23.5.0` git tag and `netapp-ontap-23.5.0.tar.gz` on Ansible Galaxy:
