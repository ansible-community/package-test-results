...
As per the [Ansible community package inclusion requirements][ci-testing], collections must pass `ansible-test sanity` tests. Version `5.0.3` of `community.network`, corresponding to the `5.0.3` tag in this repo, fails one or more of the required sanity tests.
...
``` text
plugins/modules/ce_config.py:0:0: parameter-documented-aliases-differ: Argument 'lines' in argument_spec has names 'commands', 'lines', but its documentation has names 'lines'
```
