...
As per the [Ansible community package inclusion requirements][ci-testing], collections must pass `ansible-test sanity` tests. Version `1.2.0` of `google.cloud`, corresponding to the `v1.2.0` tag in this repo, fails one or more of the required sanity tests.
...
The test `ansible-test sanity --test validate-modules` [[explain](https://docs.ansible.com/ansible-core/2.16/dev_guide/testing/sanity/validate-modules.html)] failed with 183 errors:
